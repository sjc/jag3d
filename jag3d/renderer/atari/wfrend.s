...
; include our test matrix calculator
gpumatrix:
...
; move the buffer to the end of code, meaning that the initcode doesn't get trashed,
;	so we should be able to re-use the loaded renderer for multiple objects
_GPUP1	=	endblit
_GPUP2	=	_GPUP1 + (4*SIZEOF_POLYGON)
_gpubuf = 	_GPUP2 + (4*SIZEOF_POLYGON)
