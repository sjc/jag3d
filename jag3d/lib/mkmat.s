...
;	.globl	_mkMatrix - matrix now calculated on the GPU
...
; offset arranged to make loading on the GPU easier
...
;
; testing code to add sin/cos values into an angles struct
; 	4(sp) a pointer to an angles struct
;		int alpha, beta, gamma, xpos, ypos, zpos
;

	.globl  _UpdateAngles

; TODO: implement a sin_and_cos() function in trig.s to lookup and
;	return both vales at once, as a single long

_UpdateAngles:

	move.l	4(sp),a1 	; pointer to structure

	move.w	alpha(a1), d0
	jsr		sin
	move.w	d0, alpha_sin(a1)

	move.w	alpha(a1), d0
	jsr		cos
	move.w	d0, alpha_cos(a1)

	move.w	beta(a1), d0
	jsr		sin
	move.w	d0, beta_sin(a1)

	move.w	beta(a1), d0
	jsr		cos
	move.w	d0, beta_cos(a1)

	move.w	gamma(a1), d0
	jsr		sin
	move.w	d0, gamma_sin(a1)

	move.w	gamma(a1), d0
	jsr		cos
	move.w	d0, gamma_cos(a1)
